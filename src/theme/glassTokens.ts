 ...
 *
 *  A fifth `sheer` role briefly existed for ScreenHeader, a lighter `chrome`
 *  so the DepthField could read through the header. It was removed when the
 *  header dropped its material entirely — a lighter tint still read as a band
 *  — and a role with no call sites is exactly the unexercised API this
 *  standard's own review rounds kept rejecting. */
