...
| 3 | DDoS / abuse protection | **No developer-operated endpoints.** The app has been fully local since 2026-07-07 (architecture guardrail #3): the old cloud parse proxy (`verify_jwt` gateway, per-IP rate limiting, per-user daily quota, response cache) was a **Supabase edge function** (policy in `_shared/guard.ts`, Upstash Redis REST store) that has been **removed from the repo** along with its tests. Opt-in **BYOK** (`docs/design/byok-spec.md`) is the one exception: with a user-supplied OpenAI/Anthropic key, the device calls that provider **directly** — no ProjectXavier server sits in between, so there's nothing here for us to rate-limit or protect; abuse liability on that traffic is the user's own provider account, not ours. The key lives only in the device Keychain (`AFTER_FIRST_UNLOCK` — see docs/design/byok-keychain-persist-spec.md; the earlier `WHEN_UNLOCKED_THIS_DEVICE_ONLY` silently failed to persist on device) and is never sent anywhere but that provider. If a developer-operated endpoint is ever added back, it must sit behind DDoS/WAF + rate limiting per that guardrail. |
